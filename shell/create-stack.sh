...
export KENDRA_S3_DATA_SOURCE_ID=$(aws kendra list-data-sources \
	--index-id $KENDRA_INDEX_ID \
	--region $AWS_REGION \
	--query "DataSourceSummaryList[?Name=='${STACK_NAME}-S3DataSource'].Id" --output text)

aws kendra start-data-source-sync-job \
	--id $KENDRA_S3_DATA_SOURCE_ID \
	--index-id $KENDRA_INDEX_ID \
	--region $AWS_REGION
