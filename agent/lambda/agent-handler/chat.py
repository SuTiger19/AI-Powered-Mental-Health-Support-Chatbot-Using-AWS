...
class Chat:
    ...
    def set_memory(self, event, session_id):
        ...
        self.message_history = DynamoDBChatMessageHistory(
            table_name=conversation_table_name, session_id=conversation_id
        )
        if "Human" in event:
    ...
    def get_chat_index(self):
        ...
        chat_index = dynamodb.get_item(
            TableName=conversation_index_table_name, Key=ts.serialize(key)["M"]
        )
    ...
    def increment_chat_index(self):
        ...
        dynamodb.put_item(
            TableName=conversation_index_table_name, Item=ts.serialize(input)["M"]
        )
